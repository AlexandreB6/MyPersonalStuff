 ...
 * Purge la sandbox démo du visiteur courant (films uniquement — chaque app
...
export async function POST() {
  ...
  const movies = await prisma.movie.deleteMany({ where: { demoSessionId: sid } });

  jar.delete(COOKIE_NAME);

  return NextResponse.json({ ok: true, cleared: movies.count });
}
