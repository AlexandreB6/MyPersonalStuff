 ...
 * Les infos du hover overlay sont dupliquées en sr-only pour les lecteurs d'écran.
 */
export function MovieCard({ id, title, posterPath, rating, runtime, director, cast, genres }: MovieCardData) {
  return (
    <Link
      href={`/movie/${slugify(title, id)}`}
      className="group relative flex flex-col rounded-xl overflow-hidden bg-card border border-border/50 hover:border-border hover:shadow-2xl hover:shadow-primary/5 transition-all duration-300 hover:-translate-y-1.5 cursor-pointer"
    >
      {/* Poster — negative margin eliminates sub-pixel gap with info section */}
      <div className="relative aspect-[2/3] overflow-hidden bg-card -mb-px">
        ...
        {/* Badge de note */}
        <div
          className="absolute top-2.5 right-2.5 flex items-center gap-1 bg-black/70 backdrop-blur-sm text-white text-xs font-bold px-2 py-1 rounded-full"
        ...
        {/* Hover overlay — masqué pour les lecteurs d'écran (contenu dupliqué en sr-only ci-dessous) */}
      ...
      {/* Infos réalisateur/casting accessibles aux lecteurs d'écran */}
      ...
      {/* Info */}
      <div className="relative z-10 p-3 flex flex-col gap-1.5 flex-1 bg-card">
